...
class DenseNet_features(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.n_layers = 0
        self.in_channels = 3 if kwargs['in_channels'] is None else kwargs['in_channels']
        print(self.in_channels)
        # First convolution
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(in_channels=self.in_channels, out_channels=num_init_features,
...
def densenet121_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = model_zoo.load_url(
            model_urls['densenet121'], model_dir=model_dir)
...
def densenet169_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pattern = re.compile(
            r'^(.*denselayer\d+\.(?:norm|relu|conv))\.((?:[12])\.(?:weight|bias|running_mean|running_var))$')
        state_dict = model_zoo.load_url(
            model_urls['densenet169'], model_dir=model_dir)
        for key in list(state_dict.keys()):
            '''
            example
            key 'features.denseblock4.denselayer24.norm.2.running_var'
...
def densenet201_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        pattern = re.compile(
            r'^(.*denselayer\d+\.(?:norm|relu|conv))\.((?:[12])\.(?:weight|bias|running_mean|running_var))$')
        state_dict = model_zoo.load_url(
            model_urls['densenet201'], model_dir=model_dir)
        for key in list(state_dict.keys()):
            '''
            example
            key 'features.denseblock4.denselayer24.norm.2.running_var'
...
def densenet161_features(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = model_zoo.load_url(
            model_urls['densenet161'], model_dir=model_dir)
        for key in list(state_dict.keys()):
            '''
            example
            key 'features.denseblock4.denselayer24.norm.2.running_var'
