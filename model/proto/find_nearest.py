...
import os
import time
from utils import normalize_img_rgb
import cv2
...
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for idx, (_, search_batch_input, search_y) in enumerate(dataloader):
    ...
    for j in range(n_prototypes):
        ...
        if full_save:
            ...
            for i, patch in enumerate(heaps[j]):
                ...
                patch.original_img  = normalize_img_rgb(patch.original_img) / 255
                ...
                overlayed_original_img = normalize_img_rgb(overlayed_original_img) / 255
                plt.imsave(fname=os.path.join(dir_for_saving_images,
                ...
                patch.patch = normalize_img_rgb(patch.patch) / 255
                if patch.patch.shape[0] != img_size or patch.patch.shape[1] != img_size:
                    ...
                    overlayed_patch = normalize_img_rgb(overlayed_patch) / 255
                    plt.imsave(fname=os.path.join(dir_for_saving_images,
                ...
                high_act_patch = normalize_img_rgb(high_act_patch) / 255
                np.save(os.path.join(dir_for_saving_images,
