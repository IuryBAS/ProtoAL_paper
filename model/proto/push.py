...
from model.proto.receptive_field import compute_rf_prototype
from model.proto.helpers import makedir, find_high_activation_crop
from utils import normalize_img, normalize_img_rgb
...
def push_prototypes(dataloader,  # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for push_iter, (idx_, search_batch_input, search_y) in enumerate(dataloader):
        '''
...
def update_prototypes_on_batch(search_batch_input,
    ...
    with torch.no_grad():
        search_batch = search_batch.cuda().to(torch.float)
        #print(search_y)
    ...
    for j in range(n_prototypes):
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            rf_prototype_j = compute_rf_prototype(search_batch.size(
                2), batch_argmin_proto_dist_j, protoL_rf_info)

            # get the whole image
            original_img_j = search_batch_input[rf_prototype_j[0]]
            original_img_j = original_img_j.numpy()
            original_img_j = np.transpose(original_img_j, (1, 2, 0))
            original_img_size_h, original_img_size_w = original_img_j.shape[
            ...
            proto_rf_boxes[j, 0] = rf_prototype_j[0] + \
                start_index_of_search_batch
            ...
            if prototype_network_parallel.module.prototype_activation_function == 'log':
                proto_act_img_j = np.log(
                    (proto_dist_img_j + 1) / (proto_dist_img_j + prototype_network_parallel.module.epsilon))
                ...
                proto_act_img_j = prototype_activation_function_in_numpy(
                    proto_dist_img_j)
            upsampled_act_img_j = cv2.resize(proto_act_img_j, dsize=(original_img_size_w, original_img_size_h),
            ...
            if in_channels == 1:
                # np.stack((original_img_j,)*3, axis=-1)
                original_img_j = cv2.cvtColor(
                    original_img_j, cv2.COLOR_GRAY2RGB)
            original_img_j = normalize_img_rgb(original_img_j)
            if dir_for_saving_prototypes is not None:
                ...
                if prototype_img_filename_prefix is not None:
                    ...
                    plt.imsave(os.path.join(dir_for_saving_prototypes,
                               ...
                               normalize_img_rgb(original_img_j),
                               vmin=0.0,
                               vmax=255.0)
                    # overlay (upsampled) self activation on original image and save the result
                    rescaled_act_img_j = upsampled_act_img_j - \
                        np.amin(upsampled_act_img_j)
                    rescaled_act_img_j = rescaled_act_img_j / \
                        np.amax(rescaled_act_img_j)
                    heatmap = cv2.applyColorMap(
                        np.uint8(255*rescaled_act_img_j), cv2.COLORMAP_JET)
                    heatmap = np.float32(heatmap)  # / 255
                    heatmap = heatmap[..., ::-1]
                    overlayed_original_img_j = 0.5 * original_img_j + 0.3 * heatmap
                    overlayed_original_img_j = normalize_img_rgb(
                    ...
                    if rf_img_j.shape[0] != original_img_size_w or rf_img_j.shape[1] != original_img_size_h:
                        if in_channels == 1:
                            rf_img_j = cv2.cvtColor(
                                proto_img_j, cv2.COLOR_GRAY2RGB)
                        rf_img_j = normalize_img_rgb(rf_img_j)
                        plt.imsave(os.path.join(dir_for_saving_prototypes,
                        ...
                        overlayed_rf_img_j = overlayed_original_img_j[rf_prototype_j[1]:rf_prototype_j[2],
                                                                      rf_prototype_j[3]:rf_prototype_j[4]]
                        overlayed_rf_img_j = normalize_img_rgb(
                            overlayed_rf_img_j)
                        plt.imsave(os.path.join(dir_for_saving_prototypes,
                    ...
                    if in_channels == 1:
                        proto_img_j = cv2.cvtColor(
                            proto_img_j, cv2.COLOR_GRAY2RGB)
                    proto_img_j = normalize_img_rgb(proto_img_j)
                    plt.imsave(os.path.join(dir_for_saving_prototypes,
